...
- "Levels": intentionally left as a *computed* concept (a function of unlocked achievements + health score + account age), not a stored table, to avoid a level number drifting out of sync with the achievements/score it's supposed to represent — but the actual formula wasn't picked for the same reason the health-score weighting is flagged as provisional: it's a product decision about how much each factor should matter.
