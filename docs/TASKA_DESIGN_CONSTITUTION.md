...
- The `zero_overdue_receivables` streak is defined in the catalog but not wired — it's a state check ("is the ledger currently clean"), not a discrete created-event, so it needs a scheduled daily job rather than an event hook. Noted, not built.
