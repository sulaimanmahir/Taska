...
- **Phase 8 (motion) — done (2026-08-20)**: audit was favorable here — the existing keyframe set was already restrained and on-spec, so this was "apply consistently" rather than "invent." Applied at the most central point possible rather than per-page: `.page-shell` (used by every routed page via `PageShell`/`DashboardShell`/`AppShell`) now plays the existing `fadeIn` keyframe (220ms, `prefers-reduced-motion`-guarded) on mount, so every page gets the same calm entrance with a one-line CSS change instead of 70 page edits. Separately, the existing `.skeleton` shimmer class (`shimmer` keyframe, defined since the original audit) turned out to be dead CSS — zero pages used it, while 12 files hand-rolled loading placeholders with bare Tailwind `animate-pulse` (a plainer opacity pulse, not the on-brand shimmer). Swapped the 12 leaf-level placeholder blocks (table-row/card skeletons in POS, Inventory, Debtors, Customers, Products, Partners, Dashboard, ProgressCentre, SettingsAuditLogPanel, InsightLoadingState) over to `.skeleton`. Left three patterns deliberately unchanged: `Dashboard.jsx`'s two group-wrapper pulses (a single `animate-pulse` on a parent driving several static-background children — already the same "one pulse, many children" composition `InsightLoadingState.jsx` uses correctly), `ResultsPagination.jsx`'s small pulsing dot (a live-status indicator, not a content placeholder — different semantic), and `StatsCard.jsx`'s text-level pulse (fades real text mid-refresh, not a rectangular skeleton block). Verified via Playwright: confirmed `.page-shell` computes `animation-name: fadeIn` after login, and `.skeleton` computes `animation-name: shimmer` with the expected gradient background. Full backend suite unaffected (343/343, no backend touched); frontend suite green (679/679); lint clean on every touched file.
...
- **Phase 11 (QA) — swept 2026-08-20**: an automated Playwright pass logged into a real seeded business and visited all 65 routed pages (every unique path in `App.jsx`'s route table), capturing console errors, any `5xx` API response, and a render sanity check (page body isn't suspiciously empty) per route. Found one real bug: `GET /api/reports/expenses` 500'd whenever the "Expenses by Category" breakdown ran — `ReportController::expenses()` filtered on the bare column `business_id`, and once that query joined `expense_categories` (which also has its own `business_id`), SQLite rejected it as ambiguous. Fixed by qualifying it as `expenses.business_id`. While fixing it, found a second, separate bug in the same controller: the Reports page's period selector (Today/This Week/This Month/This Year) sent `?period=...` to `sales`, `expenses`, and `profit-loss`, but all three endpoints only ever read `date_from`/`date_to` — `period` was silently ignored, so switching periods on the Reports page never changed the data shown. Added a shared `resolvePeriodRange()` helper (explicit `date_from`/`date_to` still wins; otherwise resolves `period` into a real date range, defaulting to `today` to match the page's initial state) and wired all three endpoints through it. 3 new backend feature tests (the ambiguous-column crash, and period actually changing the returned revenue), full backend suite green (345/345). Re-ran the same 65-route sweep afterward: zero problems. **Phase 12 (polish)**: not scheduled as a separate pass — polish has been happening incrementally alongside each phase above rather than being deferred to the end.
...
**Update (2026-08-12, same day): the scoring service, event wiring, and API endpoint are now built too** — the user asked to continue past the schema-only pass. What shipped:
- `BusinessHealthScoringService::computeFor()` — computes the 4 component scores + composite from real `Order`/`Expense`/`InventoryItem`/`Customer` data (30-day revenue trend vs. the prior 30 days, expense-to-revenue ratio, % of tracked inventory above its reorder point, outstanding customer balance vs. revenue). **The exact weights/formula are reasonable, documented defaults, not a settled product decision** — flagged in the class's own docblock as intentionally simple and explainable (plain average of 4 independently-reviewable 0-100 scores) so they're easy to revisit, not a black box. `computeAndStoreFor()` persists a `BusinessHealthSnapshot` per business per day.
- `GamificationService::checkAndUnlock()` — reads `config/gamification.php`, computes each metric from real data, and unlocks any newly-crossed achievement/milestone (idempotent, safe to call repeatedly).
- `GamificationObserver` (registered centrally in `AppServiceProvider::boot()`, **not** by editing `Order.php`/`Expense.php`/their controllers) — hooks `Order::created` and `Expense::created` to record the relevant streak and run `checkAndUnlock()`. Every hook is wrapped in try/catch that only logs — a bug in gamification logic can never break the actual sale or expense being saved, since it's a side effect of real business activity, never a gate on it.
- `GET /api/gamification/overview` — returns current health (score + components), active streaks, unlocked achievements/milestones, and locked-but-in-progress items with a real progress percentage (not just a name and a lock icon).
- 8 new tests across `GamificationServiceTest` and `GamificationOverviewTest`. One real Eloquent bug caught by tests before shipping: `updateOrCreate()`'s WHERE-lookup compares the raw search value against the stored column without applying the `date` cast's serialization, so a plain `Y-m-d` search string never matched an existing row (stored as a full datetime by the cast) — every call inserted a fresh row and hit the unique constraint on the second call. Fixed by an explicit `whereDate()` lookup + manual update-or-create instead of relying on the built-in helper.
- Full backend suite verified green throughout: 276 passing after this pass (was 268 after the schema-only commit).

**Update (2026-08-18): the frontend Progress Centre is built too.** `/progress`, reachable via a small icon button in the header (next to notifications) rather than a sidebar entry — mirrors how `Portfolio` and `AI Insights` are already surfaced in this codebase (contextual links, not a 28-preset nav-file edit). Sections: Business Health (composite score + 4 components), Streaks, Unlocked, and In Progress (real percent-complete bars). `lib/gamification.js` holds the pure presentation-shaping functions (8 tests). Verified live end-to-end via Playwright, not just render-smoke: created a test business, logged one ₦5,000 sale, confirmed the health score computed to 100 ("Thriving"), the daily-sales streak showed 1 day, "First Sale" and "Debt Free" achievements unlocked with today's date, and in-progress items showed correct real percentages (e.g. "Growing Footprint" at 50% with 1 of 2 branches) — then cleaned up the test business.

**Update (2026-08-18): the zero_overdue_receivables streak is wired too.** Added `taska:compute-gamification-snapshots` (`backend/routes/console.php`, matching the existing inline-`Artisan::command` convention already used by `taska:repair-business-state` — no new `app/Console/Commands` class structure introduced) — for every business, stores a `BusinessHealthSnapshot` and extends the `zero_overdue_receivables` streak only when the business currently has zero outstanding customer balance. Registered on a daily schedule via `bootstrap/app.php`'s `withSchedule()` (`dailyAt('02:00')`) — note this declares the schedule but doesn't activate it: Laravel's scheduler only actually fires if a real cron entry on the server runs `php artisan schedule:run` every minute, which is a deployment/ops decision outside this codebase, not something code alone turns on. 2 new tests (`GamificationSnapshotCommandTest`), full backend suite green at 278.

**Update (2026-08-20): the levels formula shipped.** Stayed a *computed* value, never a stored column, for the reason noted above — a stored level number would drift out of sync with the achievements/score it's supposed to represent. `GamificationService::computeLevel()` is a pure function (no queries of its own — the controller passes in already-computed values) using the same "reasonable, documented default, not a settled product decision" framing as the health-score weights: 100 points per level, from unlocked achievements (15pts each) + unlocked milestones (30pts each, since they represent bigger wins) + up to 30pts from the current health score + up to 20pts from account age (1pt per 2 weeks, capped at a year). `GET /api/gamification/overview` now returns a `level` object (`level`, `points`, `points_into_level`, `points_to_next_level`) alongside `health`; the Progress Centre gained a Level card with a progress bar toward the next level. 6 new backend tests (the pure formula plus the overview endpoint) and 2 new frontend tests, full backend suite green (349/349), frontend green (681/681). Verified live via Playwright: a business with 2 unlocked achievements and a 100 health score showed Level 1 with the progress bar at the exact predicted 60%.

**Still not built** (deliberately, still open):
- Actually wiring the deployment's real cron to call `php artisan schedule:run` — an infra/ops task, not a code task.
...
## Real design tokens supplied (2026-08-21) — reverses this doc's own v2.0 palette decision

Worth recording plainly rather than quietly overwriting: §1's note above says v1.0 of this constitution used a distinct "Result Seekers Purple #4214C4" palette, the user explicitly rejected it, and v2.0 (this document, as originally written) replaced it with a Makaranta-shared palette instead — "never introduce a competing colour palette for Taska," §77 originally said. On 2026-08-21 the user supplied a new, separate, detailed visual-design-system specification that reinstates `#4214C4` as "the Result Seekers brand colour... the primary visual anchor... do not replace it with another purple," and explicitly says not to wait for or reference Makaranta at all. That's a second reversal of the same decision, not a misunderstanding on either side - this is the user's latest, explicit, direct instruction in this conversation, so it takes precedence and has been implemented as such. Flagging the history here so nobody mistakes the sections above (§1, §69, §77, §294–298) for still-current guidance on the palette question specifically; everything else in this document (motion philosophy, spacing discipline, component craftsmanship standard, "match the craftsmanship, not the screens") still stands unchanged.

**Phase 2 (token foundation) is implemented and live** in `frontend/src/index.css`:

- Tailwind's own `violet`/`purple`/`orange` palettes are remapped via a `@theme` block to the new RS scale — this alone fixed the ~64 files that bypassed the CSS-variable tokens with bare `bg-violet-600`-style utilities (the single biggest risk the original 2026-08-11 audit flagged), with zero per-file edits.
- New semantic tokens (`--background`/`--surface`/`--foreground`/`--border` tiers, `--primary`/`--accent`, status colors) added alongside the existing `--color-*` names (which now alias to them), converging naming without a big-bang rename across 70+ pages.
- Spacing (`--space-*`), radius (`--radius-*`, plus retuned `--card-radius`/`--field-radius` to the spec's more restrained 14px/10px), shadow (`--shadow-xs..xl`, retuned to the spec's lighter values), and motion (`--motion-*`) scales added.
- Font switched from "Plus Jakarta Sans" to Inter — this also fixed a **real latent bug**: "Plus Jakarta Sans" was declared as the primary font everywhere but was never actually loaded anywhere (no npm package, no `@font-face`, no Google Fonts link, no bundled files), so the entire app had silently been rendering in whatever fallback the browser picked. Inter is now genuinely loaded via a Google Fonts `<link>` in `index.html`.
- Verified live via Playwright: the primary button's computed background is `rgb(66, 20, 196)` — exactly `#4214C4`.

**A real light/dark theme engine now exists too** (the user's separate, follow-up dark-mode/logo specification), scoped to its own genuinely-achievable foundation rather than the full 14-phase spec in one pass:
- Dark token layer (`:root[data-theme="dark"]` in `index.css`) mirroring every light semantic token with the spec's dark values.
- `stores/themeStore.js` (Zustand, matching `authStore`/`offlineStore`'s existing pattern) with `light`/`dark`/`system` preference, `localStorage` persistence, and a live listener for OS theme changes while on `system`.
- A no-flash boot script inline in `index.html` that resolves and applies `data-theme` before React mounts (this app is a JS-required SPA, so no separate no-JS CSS fallback was needed).
- `ThemeToggle.jsx`, wired into Settings → Profile next to the existing push-notification toggle.
- 3 new store tests (`tests-render/themeStore.test.jsx`), full frontend suites green (685 + 65 render-smoke).

**What this foundation does NOT yet cover** — confirmed by live Playwright screenshots in both themes, not assumed: pages built on the token-driven shared components (`PageShell`, `Card`, `Sidebar`) flip to dark correctly (sidebar, KPI cards, page background all verified correct), but plenty of individual page content still uses hardcoded light-mode-only Tailwind utility classes (`text-slate-900`, `bg-white`, etc.) instead of the semantic tokens, so those spots stay low-contrast in dark mode — e.g. the topbar workspace label and page eyebrow text were visibly hard to read in the dark-mode screenshot. This is exactly the "Phase 5–8: convert shared components, then admin/client/specialized modules" work the design system spec itself describes as separate, larger phases — not attempted here, and not safe to blanket-fix without per-page verification the same way the token-bypass sweep will eventually need. Also not attempted: the `TaskaLogo`/`ClientLogo` component system with surface-aware variant selection and a contrast engine, client logo upload/storage (a real new backend feature — model, migration, upload endpoint, validation), print-safe styles, and syncing the theme preference to the user's account server-side (currently device-local via `localStorage` only). All tracked here as genuine, scoped follow-up, not silently dropped.
