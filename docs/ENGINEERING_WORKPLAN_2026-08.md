...
6. **Frontend test coverage** — only 2 test files existed against 73 `lib/*.js` files and 68 pages, despite the `lib/` layer being cleanly testable pure functions.
...
### 6. Frontend test coverage — Not started
- [ ] Backfill unit tests for `lib/` modules opportunistically as they're touched (not a big-bang effort)
- [ ] Prioritize modules backing money-moving flows first (finance, inventory, orders) over display-only modules
