...
Status: **steps 1-4 of the suggested sequencing below are implemented and verified (portfolio dashboard, `active_business_types` schema + self-serve endpoint, `modules` edit endpoint/settings UI, navigation/routing merge). Only step 5 (billing model) remains, and it's an explicitly separate product decision, not an engineering task.**
...
3. **`modules` edit endpoint + settings UI** — lets a business toggle feature flags within its active verticals; independently useful even before multi-vertical support ships. **Done (2026-08-07).** `GET/PATCH /api/auth/business/modules` (`AuthController`) lets a business member list and toggle modules within their current `business_type`, validated against `config('business_types.types.{type}.modules')`; `dashboard` is force-kept enabled regardless of what's submitted, since every vertical's module list starts with it and disabling it would break navigation. Frontend: new "Modules" tab on Settings (`SettingsModulesPanel.jsx`) with humanized checkboxes. One real bug caught via live verification before shipping: the first version of the "has changes" check compared derived state against itself (tautological, could never detect a change, Save button stayed permanently disabled) - fixed by tracking last-saved state separately from the live draft. Covered by 5 backend feature tests, added to the Vitest render-smoke suite, verified via a live end-to-end toggle-save-reload check.
