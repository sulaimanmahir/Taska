...
4. **Frontend navigation/routing merge** — the real work; touches `useBusinessType()` and the nav-preset merge logic; every page's `if (type === 'x')` branch needs auditing against "is X in active_business_types" instead. **In progress (2026-08-06) — navigation merge done, per-page branching audit not started.** `getMergedNavigationForBusinessTypes()` (`navigationPresets.js`) combines nav sections across every active vertical, deduping shared items (Dashboard, Reports, etc.) by path while preserving vertical-specific items; for the single-vertical case (every real business today) it returns an identical result to the existing single-type function, so this shipped as a no-op in practice. `BusinessTypeProvider` now reads `business.active_business_types` and exposes `activeTypes`/`hasActiveType()` on context for future page-level use. Verified via 5 new unit tests, the full node suite (604 passing), the render-smoke suite (13 passing), and a live end-to-end check (registered a real account, added a second vertical via the API, confirmed `active_business_types` flows through to the payload the frontend reads). **Still not done: auditing the ~68 pages' individual `if (type === 'x')` branches** — deliberately deferred, since no real business has 2+ active types yet (no settings UI exposes the self-serve endpoint), so this can be done incrementally, page by page, as it actually becomes load-bearing rather than as one large mechanical sweep now.
