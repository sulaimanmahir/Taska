...
4. **Frontend navigation/routing merge** — the real work; touches `useBusinessType()` and the nav-preset merge logic; every page's `if (type === 'x')` branch needs auditing against "is X in active_business_types" instead. Not started — deliberately deferred out of the step-2 pass; this is the largest, highest-blast-radius piece of the whole proposal (touches ~68 pages) and deserves its own dedicated pass with a clear plan for how to test it incrementally rather than as one big-bang change.
