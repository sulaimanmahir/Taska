...
- **`AdminController` rebuilt on real models (2026-08-20)**, closing the gap the entry above deliberately left open. Rewired `stats`, `users`, `businesses`, `plans`, and `transactions` onto the real billing models (`BusinessSubscription`, `Invoice`, `SubscriptionPlan`) and `referrals` onto the real referral models (`ReferralCommission`, `ReferralAgent`) — `plans` was also querying the wrong table name (`plans` instead of `subscription_plans`). Added a derived `Business::owner()` (earliest-joined admin-role member — there's no stored "owner" concept in the schema) used by both `businesses` and `transactions`. `supportTickets`/`resolveTicket` are a genuine gap, not a wiring bug — no support-ticket model/table exists anywhere in this codebase — so those now return an honest empty list / a clear 501 instead of fatal-erroring or faking data; a real ticket system is separate, larger, unstarted work. Separately found and fixed: `/admin` was the only route in the entire app not wrapped in `ProtectedRoute`, so on any direct/hard navigation (the *only* way anyone reaches it, since it isn't linked from the nav) the page redirected away before the user's profile ever finished loading — made this whole rebuild practically unreachable until fixed. 9 new backend feature tests, verified live via Playwright (all 6 tabs rendering real data against a seeded test business). Full backend suite still green (339/339).
